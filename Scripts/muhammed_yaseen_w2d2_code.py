...
if uploaded_file is not None:
    ...
    with tab2:
        st.info("Category charts arrive on Day 7.")
        ...
        sub_cat_sales = df.groupby("sub_category").agg({
                                                            'sales':'sum',
                                                            'profit':'sum'
                                                        }).sort_values(by='sales', ascending=False)
        st.dataframe(sub_cat_sales.style.format("${:,.0f}"),use_container_width=True)
        st.bar_chart(sub_cat_sales)
        
    with tab3:
        st.info("Region charts arrive on Day 7.")
